...
- The desktop invitation gap is closed. Owner sessions retain their original credential only as an in-memory, expiring HTTP administration capability after its WebSocket capability is redeemed. The panel can mint named editor or viewer invitations; explicit bounded JSON encoding is used only for user-directed clipboard transfer, and the Join Session dialog revalidates the selected editor both after modal input and after network synchronization. A real embedded-service test proves owner creation, editor invitation, a second authenticated client reaching `Caught up`, rejection of invitation reuse, and credential clearing on leave.
...
- Create-local, invitation creation, paste-to-join, and leave actions are attached to the selected/open map. Reconnect uses a rotating, dedicated resumption credential and starts automatically after transport loss. The new invitation controls and paste dialog compile and pass headless lifecycle tests but still require the two-process Dear ImGui playtest below.
- The collaboration panel shows session identity, role, revision, explicit synchronization state, participants, named editor/viewer invitation controls, bounded conflict text, errors, retry reconnect, and leave. Retry reconnect remains disabled while an automatic attempt is active and becomes available after transient attempts are exhausted.
