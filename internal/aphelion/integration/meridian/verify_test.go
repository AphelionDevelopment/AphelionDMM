...
func newVerifyFixture(t *testing.T, client Client, runner AcceptanceRunner) (*AcceptanceVerifier, integrationmanifest.Manifest, string) {
	t.Helper()
	root := t.TempDir()
	...
	staged := filepath.Join(stageRoot, "candidate.dmm")
	...
	verifier, err := NewAcceptanceVerifier(VerifierConfig{
		Repository: Repository{Identity: "Meridian-Rift", Root: root, DME: "tgstation.dme", Targets: map[string]string{"test-map": filepath.Join(".aphelion-stages", "candidate.dmm")}},
